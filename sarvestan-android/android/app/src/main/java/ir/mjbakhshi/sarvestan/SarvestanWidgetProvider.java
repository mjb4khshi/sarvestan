...
public class SarvestanWidgetProvider extends AppWidgetProvider {
    ...
    public static void updateOne(Context context, AppWidgetManager mgr, int id) {
        ...
        if (gpa != null && !gpa.isEmpty() && !gpa.equals("ـ") && !gpa.equals("-")) {
            Bitmap bGpa = WidgetTypographyHelper.renderText(context, "معدل " + toFaDigits(gpa), 11f, Color.parseColor("#F1F5F9"), WidgetTypographyHelper.getBold(context), android.graphics.Paint.Align.LEFT);
            if (bGpa != null) rv.setImageViewBitmap(R.id.w_combo_gpa_img, bGpa);
        }

        // 2. کلاس بعدی
        Bitmap bClassBadge = WidgetTypographyHelper.renderText(context, "کلاس بعدی", 10f, Color.parseColor("#38BDF8"), WidgetTypographyHelper.getMedium(context), android.graphics.Paint.Align.RIGHT);
        ...
        Bitmap bCTitle = WidgetTypographyHelper.renderText(context, cTitle, 13f, Color.parseColor("#F8FAFC"), WidgetTypographyHelper.getBold(context), android.graphics.Paint.Align.RIGHT);
        ...
        Bitmap bCMeta = WidgetTypographyHelper.renderText(context, meta, 10f, Color.parseColor("#94A3B8"), WidgetTypographyHelper.getRegular(context), android.graphics.Paint.Align.RIGHT);
        if (bCMeta != null) rv.setImageViewBitmap(R.id.w_combo_class_meta, bCMeta);

        // 3. امتحان
        if (examTitle == null || examTitle.isEmpty() || examDays < 0) {
            Bitmap bETitle = WidgetTypographyHelper.renderText(context, "امتحانی نزدیک نیست", 12f, Color.parseColor("#F8FAFC"), WidgetTypographyHelper.getBold(context), android.graphics.Paint.Align.RIGHT);
            if (bETitle != null) rv.setImageViewBitmap(R.id.w_combo_exam_title, bETitle);

            Bitmap bEMeta = WidgetTypographyHelper.renderText(context, "روزشمار خالی", 10f, Color.parseColor("#94A3B8"), WidgetTypographyHelper.getRegular(context), android.graphics.Paint.Align.RIGHT);
            if (bEMeta != null) rv.setImageViewBitmap(R.id.w_combo_exam_meta, bEMeta);

            Bitmap bEDays = WidgetTypographyHelper.renderTextCenter(context, "—", 11f, Color.WHITE, WidgetTypographyHelper.getBold(context));
            if (bEDays != null) rv.setImageViewBitmap(R.id.w_combo_exam_days, bEDays);
        } else {
            Bitmap bETitle = WidgetTypographyHelper.renderText(context, examTitle, 12f, Color.parseColor("#F8FAFC"), WidgetTypographyHelper.getBold(context), android.graphics.Paint.Align.RIGHT);
            if (bETitle != null) rv.setImageViewBitmap(R.id.w_combo_exam_title, bETitle);

            Bitmap bEMeta = WidgetTypographyHelper.renderText(context, toFaDigits(examDate == null ? "" : examDate), 10f, Color.parseColor("#94A3B8"), WidgetTypographyHelper.getRegular(context), android.graphics.Paint.Align.RIGHT);
            if (bEMeta != null) rv.setImageViewBitmap(R.id.w_combo_exam_meta, bEMeta);

            String daysStr = examDays == 0 ? "امروز" : examDays == 1 ? "فردا" : (toFaDigits(String.valueOf(examDays)) + " روز");
            Bitmap bEDays = WidgetTypographyHelper.renderTextCenter(context, daysStr, 11f, Color.WHITE, WidgetTypographyHelper.getBold(context));
            if (bEDays != null) rv.setImageViewBitmap(R.id.w_combo_exam_days, bEDays);
        }
